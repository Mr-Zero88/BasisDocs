...
A movement group rides the component's own `enabled` state. Anything that flips the component on or off — including an [HVR.Vixxy](/docs/avatar/constraints) activation — turns its movements on and off, with no per-frame polling. Group the movements that should toggle together onto one component.
